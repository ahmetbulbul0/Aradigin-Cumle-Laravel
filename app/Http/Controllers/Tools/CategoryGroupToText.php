...
class CategoryGroupToText extends Controller
{
    static function single($no) {
        ...
        $data = [
            "main" => $get["main"]["name"],
            "sub1" => $get["sub1"]["name"] ?? "deneme1",
            "sub2" => $get["sub2"]["name"] ?? "denem4",
            "sub3" => $get["sub3"]["name"] ?? NULL,
            "sub4" => $get["sub4"]["name"] ?? NULL,
            "sub5" => $get["sub5"]["name"] ?? NULL
        ];
        if (empty($data["sub1"])) {unset($data["sub1"]);}
        if (empty($data["sub2"])) {unset($data["sub2"]);}
        if (empty($data["sub3"])) {unset($data["sub3"]);}
        if (empty($data["sub4"])) {unset($data["sub4"]);}
        if (empty($data["sub5"])) {unset($data["sub5"]);}
